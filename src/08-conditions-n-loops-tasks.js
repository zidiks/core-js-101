...
function doRectanglesOverlap(rect1, rect2) {
  function Rect(x) {
    this.x = x.left;
    this.y = x.top;
    this.x1 = x.left + x.width;
    this.y1 = x.top + x.height;
  }
  const a = new Rect(rect1);
  const b = new Rect(rect2);
  if (a.y > b.y1 || a.y1 < b.y || a.x1 < b.x || a.x > b.x1) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  const x = Math.sqrt((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
  if (x < circle.radius) return true;
  return false;
}
